...
We're building a hybrid classifier: a CLIP branch for the high-level "does this look real" stuff, and a small CNN over the FFT spectrum for the low-level generator-fingerprint stuff, fused with a small MLP and a learned temperature so we get an actual calibrated probability out, not just a ranking score. This is basically what the brief itself pointed us at ("Key Insight: Go hybrid" — combine CLIP semantics with frequency patches because they survive different kinds of image damage), so we're not reinventing anything here, just building what was suggested.

Why not just fine-tune CLIP and call it done? Because the brief warns about this directly too — "don't just fine-tune a classifier, think about what your model is actually learning." A CLIP-only model can cheat by picking up on semantic giveaways that the next generator update just fixes, and it has no mechanism at all for catching frequency-domain artifacts, which tend to be a lot more durable. The frequency branch is cheap to add (4 conv layers), so there wasn't really a reason not to.

## Data

- **WildFake** is our actual training set. Multi-generator, which matters a lot since generalizing across generators is basically the hardest part of this whole challenge.
...
The idea from the brief: if a transformation can happen to an image on a real feed — recompression, blur, someone screenshots a screenshot, cropping for a thumbnail — it has to happen during training too, or the model picks up on signals that don't survive contact with the real world. So `src/data/augmentations.py` does JPEG recompression at random quality, blur, crop-then-resize instead of plain downsampling (SAFE paper's tip — cropping keeps the high-frequency detail that downsampling destroys), color jitter + rotation, sensor noise, and a simulated re-screenshot.
...
Primary metric is ROC AUC — threshold-free, doesn't care about class imbalance. We build a transformed test set (JPEG at a few qualities, blur, crop, and more we added once we realized our first pass didn't cover everything the brief actually asks for — see the real results in the main README) and compute:
...
- Heavy augmentation costs a bit of clean-set accuracy. Worth it — a model that's great on clean images and falls apart under JPEG compression isn't really solving what the brief is asking for.
...
- Two branches, not some four-branch FFT+DCT+PRNU setup. Something that actually finishes training on hackathon compute and has a working demo beats a fancier idea that doesn't run in time.
